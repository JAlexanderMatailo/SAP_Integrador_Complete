...
{
    public class ComercialManager :BaseManager
    {
        ...
        public OrdenVentaViewModel GetOrdenComercial(int DocEntry, string CompanyDB)
        {
        ...
        //public OrdenVentaViewModel GetOrdenBLComercial(string NumeroBL, string TipoMovimiento,  string CompanyDB)
        //{
        //    if (!servicio.IsConected)
        //        servicio.ConectarSL(CompanyDB);
        ...
        public List<OrdenesVentaCancelarItem> GetOrdenesVentaBls(string NumeroBL, string TipoMovimiento, string CompanyDB)
        {
            HanaCommand comm = null;
            HanaDataReader reader = null;
            List<OrdenesVentaCancelarItem> ordenes = new List<OrdenesVentaCancelarItem>();
            OrdenesVentaCancelarItem orden = new OrdenesVentaCancelarItem();

            try
            {
                conectarHana(CompanyDB);
                string StrQty = "SELECT T0.* FROM \"" + CompanyDB + "\".\"CTK_GET_OV_BL_VIEW\" ";
                  StrQty += " T0 WHERE T0.\"U_EXX_TIPO_TRANSACC\" like '" + NumeroBL + "' AND T0.\"U_EXX_DOC_GEN\" like '" + TipoMovimiento +"'" ;
                ...
                if (reader != null)
                {
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            orden = new OrdenesVentaCancelarItem();
                            orden.DocEntry = Convert.ToInt64(reader.GetValue(0));
                            orden.U_EXX_TIPO_TRANSACC = Convert.ToString(reader.GetValue(1));
                            orden.U_EXX_DOC_GEN = Convert.ToString(reader.GetValue(2));
                            orden.DocStatus = Convert.ToString(reader.GetValue(3));
            ...
            {
                List<OrdenesVentaCancelarItem> ordenesError = new List<OrdenesVentaCancelarItem>();
                OrdenesVentaCancelarItem ordenError = new OrdenesVentaCancelarItem();
        ...
        public RespSocioNegocio GetAliasSocioNegocio(string Ruc, string CompanyDB)
        {
            HanaCommand comm = null;
            HanaDataReader reader = null;
            List<OrdenesVentaCancelarItem> ordenes = new List<OrdenesVentaCancelarItem>();
            OrdenesVentaCancelarItem orden = new OrdenesVentaCancelarItem();
            string Alias = "";
            RespSocioNegocio socio = new RespSocioNegocio();
            try
            {
                conectarHana(CompanyDB);
                string StrQty = "Select  T0.\"AliasName\" Alias from \""+ CompanyDB + "\".\"OCRD\" T0  ";
                StrQty += "where T0.\"CardType\" = 'C' and T0.\"LicTradNum\" = '" + Ruc + "'";
           ...
           //     StrQty += " T0 WHERE T0.\"U_EXX_TIPO_TRANSACC\" like '" + NumeroBL + "' AND T0.\"U_EXX_DOC_GEN\" like '" + TipoMovimiento + "'";
                ...
                if (reader != null)
                {
                    if (reader.HasRows)
                    {
                        ...
                        {
                          //  orden = new OrdenesVentaCancelarItem();
                           Alias= Convert.ToString(reader.GetValue(0));
                            //orden.U_EXX_TIPO_TRANSACC = Convert.ToString(reader.GetValue(1));
                            //orden.U_EXX_DOC_GEN = Convert.ToString(reader.GetValue(2));
                            //orden.DocStatus = Convert.ToString(reader.GetValue(3));
                            socio.Alias = Alias;
                    ...
                    {
                        socio.Mensaje = "No se encontro cliente con el Ruc " + Ruc;
                    }
                }
            }
            catch (Exception ex)
            {
                socio.Mensaje = ex.Message;
                return socio;
            }
            finally
            {
        ...
        //                    this.errMsg = $"Error al crear EXX_PUERTOS DESTINO: {obj.U_EXX_PDEST}";
        //                    respuestaPrincipal.Success = false;
        //                    respuestaPrincipal.ErrMensaje = this.errMsg;
        //                    return respuestaPrincipal;
        ...
        //            U_EXX_COD_AG = "", //RUC de un proveedor tipo agente se usará en el flujo comercial,Enviamos el RUC del proveedor grabar en Sap con su respectivo Código
        //            U_EXX_DES_AG = "", // Razón social del proveedor se usará en el flujo comercial
        //            U_EXX_CVEND = "", //RUC del Vendedor se usará en el flujo comercial 
        //            U_EXX_DVEND = "", //Nombre del Vendedor se usará en el flujo comercial
        //            U_EXX_EMB = "",//Razón social del Embarcador 
        //            U_EXX_CONS = "",//Razón social del Embarcador 
        //            U_EXX_NUM_COT = "", //Código secuencial indica cual es numero de cotización se usará en el flujo comercial CT-00000021-17
        //            U_EXX_ROPOPA = obj.U_EXX_ROPOPA, //Debe ser el mismo del campo Código.
        //            U_EXX_MBL = "",//Numero de MBL (Master BL) asociado al BL
        //            U_EXX_HBL = "",//Numero de HBL (Bl Hijo) asociado al BL
        ...
        public RespuestaGenerica CancelarOrdenComercial(OrdenesVentaCancelarItem obj, string CompanyDB)
        {
            if (!servicio.IsConected)
                servicio.ConectarSL(CompanyDB);
            System.Net.HttpStatusCode _Status = System.Net.HttpStatusCode.Created;
        ...
        public RespCostoLocal GetItemCodeMapeoCostoLocal(string CodigoCL, string CompanyDB)
        {
            HanaCommand comm = null;
            HanaDataReader reader = null;
          //  List<OrdenesVentaCancelarItem> ordenes = new List<OrdenesVentaCancelarItem>();
          //  OrdenesVentaCancelarItem orden = new OrdenesVentaCancelarItem();
            string ItemCode = "", SuppCatNum="";
            RespCostoLocal costo = new RespCostoLocal();
            try
            {
                conectarHana(CompanyDB);
                string StrQty = "Select  T0.\"ItemCode\" ," + "T0.\"SuppCatNum\" " + " from \"" + CompanyDB + "\".\"OITM\" T0  ";
                StrQty += "where T0.\"SuppCatNum\" = '" + CodigoCL + "'";
                ...
                if (reader != null)
                {
                    if (reader.HasRows)
                    {
                        ...
                        {
                            //  orden = new OrdenesVentaCancelarItem();
                            ItemCode = Convert.ToString(reader.GetValue(0));
                            SuppCatNum = Convert.ToString(reader.GetValue(1));
                            //orden.U_EXX_TIPO_TRANSACC = Convert.ToString(reader.GetValue(1));
                            //orden.U_EXX_DOC_GEN = Convert.ToString(reader.GetValue(2));
                            //orden.DocStatus = Convert.ToString(reader.GetValue(3));
                            costo.CodigoCL = CodigoCL;
                            costo.ItemCode = ItemCode;
                            costo.SuppCatNum = SuppCatNum;
                            costo.Mensaje = "";
                            // ordenes.Add(orden);
                        }
                    ...
                    {
                        costo.Mensaje = "No se encontro cliente con el Ruc " + CodigoCL;
                    }
                }
            }
            catch (Exception ex)
            {
                costo.Mensaje = ex.Message;
                return costo;
            }
            finally
            {
        ...
        public RespuestaGenerica GuardarOrdenVentaComercial(OrdenVentaGuardarRequest obj, string CompanyDB)
        {
            ...
            { //OBTENER CARDCODE
                ...
                if ( docasoc.Value == 3)
                {
                    ...
                    respuestaPrincipal.Success = false;
                    respuestaPrincipal.ErrCodigo = -001;
                    respuestaPrincipal.ErrMensaje = $"Documento Asociado  {obj.U_EXX_TIPO_TRANSACC} no pudo ser creado. Ya existen 3 documentos asociados para el BL";
                    return respuestaPrincipal;
                ...
                foreach (var item in obj.DocumentLines)
                {
                    ...
                    if (obj.U_EXX_DOC_GEN== "FacturaCostosLocales")
                    {
                        var mapeolocal = GetItemCodeMapeoCostoLocal(item.ItemCode, CompanyDB);

                        if (mapeolocal == null)
                        {
                            respuestaPrincipal.Success = false;
                            respuestaPrincipal.ErrCodigo = -002;
                            respuestaPrincipal.ErrMensaje = $"Artículo o Item  con [{item.ItemCode}] no existe en SAP para costos locales.";
        ...
        public RespuestaGenerica GuardarOrdenVentaTurnos(OrdenVentaGuardarRequest obj, string CompanyDB)
        {
            try
            {
                string IVA_SAP = ConfigurationManager.AppSettings["IVA"].ToString();//http://192.168.238.2:9097/";//
                ...
                { //OBTENER CARDCODE
                    ...
                    foreach (var item in obj.DocumentLines)
                    {
                        ...
                        item.TaxCode = itemsap?.ArTaxCode?.ToString() == "" ? "IVA_EXE" : itemsap?.ArTaxCode?.ToString();

                        //Valida si esxiste proyecto
                        var proyecto = catalogos.GetProyectoByCode(item.ProjectCode, CompanyDB);

                        if (proyecto.ErrCodigo == 404)
                        {
                            ProjectsGuardarViewModel proj = new ProjectsGuardarViewModel();
                            proj.Code = item.ProjectCode;
                            proj.Name = item.NombreProjectCode;
                            DateTime now = DateTime.Now.AddDays(-7);
                            proj.ValidFrom = now.AddDays(-7).ToString("yyyy-MM-dd");
                            proj.ValidTo = now.AddYears(5).ToString("yyyy-MM-dd");
                            var nuevoProyecto = catalogos.GuardarProyecto(proj, CompanyDB);
                            if (!nuevoProyecto.Success)
                            {
                                return nuevoProyecto;
                            }
                        }
                        //if (proyecto == null || proyecto.ErrCodigo == 404)
                        //{
                        //    respuestaPrincipal.ErrCodigo = proyecto.ErrCodigo;
                        //    respuestaPrincipal.ErrMensaje = $"El Código Proyecto (Asesor Comercial) [{item.ProjectCode}] no existe en SAP.";
                        //    return respuestaPrincipal;
                        //}
                        if (proyecto == null || proyecto.ErrCodigo == 400)
                        {
                            respuestaPrincipal.ErrCodigo = 400;
                            respuestaPrincipal.ErrMensaje = proyecto.ErrMensaje;
                            respuestaPrincipal.Success = false;
                            return respuestaPrincipal;
                        }
                        ...
                        for (int i = 1; i <= 5; i++)
                        {
                            ...
                            if (!string.IsNullOrEmpty(cc))
                            {
                                ...
                                if (centrocosto == null)
                                {
                                    ...
                                    profit.CenterCode = cc;
                                    //profit.CenterName = $"{cc}/{obj.U_EXX_TIPO_TRANSACC}";
                                    profit.CenterName = $"{cc}";
                                    profit.GroupCode = "1";
                                    profit.InWhichDimension = 1;
                                    //profit.EffectiveTo = DateTime.Now.AddDays(-30);
                                    profit.EffectiveFrom = DateTime.Now.AddDays(-30);
                                    RespuestaGenerica respuestaprof = catalogos.GuardarCentroCostos(profit, CompanyDB);
                                    if (!respuestaprof.Success)
                                    {
                                        respuestaprof.ErrMensaje += $"Centro de costos para el dato [{cc}] no pudo ser creado. ";
                                        return respuestaprof;
                                    }
                                    //ProfitCenterViewModel centrocosto = catalogos.GetCentroCostos(cc, CompanyDB);
                                    //respuestaPrincipal.Success = false;
                    ...
                    obj.CancelDate = obj.DocDate;
                    obj.DocDueDate = obj.DocDate;
                    //obj.U_LocalCtaContab = bp.Properties2;

                    var _IVA_CLIENTE = bp.Properties4;
